...
def bug_handler(bug_queue, udid):
    log_dir = f"D:\\rltest\\ARES-pytorch-ares\\rl_interaction\\re"
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    log_file_path = os.path.join(log_dir, "all_logs.txt")

    os.system(f'{adb_path} -s {udid} logcat -c')

    with open(log_file_path, "w", encoding='utf-8') as log_file:
        proc = subprocess.Popen([adb_path, '-s', udid, 'logcat'], stdout=log_file)
